...
Although it's impossible to have a network that is "unhackable", the first step is to not purchase $50 IP cameras that ship with a bunch of security flaws. Regardless of their price, avoiding IP cameras in most situations is the most viable option. CCTV cameras are safer as their feed is physically transmitted over analog and everything is local on your own local storage. Nothing is going over the internet or your Wi-Fi. 

If you have to use an IP Camera, invest money into a reputable camera that doesn't literally come shipped with admin panel CVE's. In other words, one that doesn't come shipped with known security flaws. Just google the name of the camera with "exploit" appended at the end. If you're finding CVE's or results from the sites "exploitdb" or "rapid7", definitely don't buy that camera. You can also segment the camera's network into its own subnet so that even if it's compromised, it can't communicate with your main network.

### Hardening the Camera

If an IP camera is your most valuable option, then hardening it is a great idea. A lot of IP cameras come with stock credentials. These are default admin credentials such as "admin:admin" or "admin:password". You obviously should change these. Only purchase cameras that allow you to password protect the video feed. This means that if someone dorks your feed, surely they can connet to it, but they won't know the password in able to view the feed. All passwords should follow basic password protocols in terms of complexity to reduce the chance of bruteforce attacks. Make it something that isn't easy to guess, and make it a password that isn't used for anything else. 

Due to IP Cameras being useful for their capability to be accessed anywhere remotely, many cameras will tell you to open up the port the camera is running on so that you can access it from outside of your network. If you can, don't open the port. This will prevent people from remotely connecting to it in the first place. If you're never going to access your camera feed when you're not home or if you can live without it, then skip this all together as it will save you 99% of the troubles. The only thing that can happen now is if someone locally hacks your camera physically in person by jacking into your network while war driving or something stupid like that but that's way out of scope of this post and we're getting off topic.
...
If you want a more secure camera, you can trade off your data for better security by purchasing a home security package from a company such as Simplisafe. They collect some of your data and your account info, alerts, videos, etc. are stored on their cloud servers, but you get WAY better security. For a home security route, this is the way to go other than maybe CCTV if that's convenient to you. CCTV has some downsides and only really have privacy advantages, everything else makes them inconvenient. With CCTV you have to hook up analog and route that through your home, and hook it up to a DVR or your own HDD. Companies such as simpisafe offer more expensive packages overall but are more ideal and secure. Also, if you purchase cameras, don't be that person who puts them in private areas in your home.